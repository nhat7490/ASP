...
import Foundation
import UIKit
class RoomDetailForFinderVC:BaseVC,UIScrollViewDelegate,OptionViewDelegate,UtilitiesViewDelegate{
...
//    
    ...
    var viewType:ViewType = .detailForFinder
...
//        navigationController?.navigationBar.topItem?.backBarButtonItem =
        navigationController?.navigationItem.leftBarButtonItem = UIBarButtonItem(title: "BACK".localized, style: .done, target: self, action: #selector(onClickBackButton))
...
//        _ = membersView.anchor(genderView.bottomAnchor, contentView.leftAnchor, nil, contentView.rightAnchor, padding,CGSize(width: 0, height: membersViewHeight))
    ...
    func setData() {
        ...
        print(navigationController?.navigationItem.leftBarButtonItem?.title)
...
//        membersView.lblNumberOfMemberAdded.text = String(format: "ROOM_DETAIL_ADDED_NUMBER_OF_MEMBER".localized,room.users?.count ?? 0)
        ...
        optionView.lblTop.attributedText = NSAttributedString(string: String(format: "PRICE_OF_ROOM".localized, room.minPrice!,"MONTH".localized), attributes: [NSAttributedStringKey.foregroundColor:UIColor.red])
        optionView.lblBottom.attributedText = NSAttributedString(string: String(format: "TIME".localized, room.minPrice!,"MONTH".localized), attributes: [NSAttributedStringKey.foregroundColor:UIColor.red])
        view.layoutIfNeeded()
    }
    ...
    func optionViewDelegate(view optionView: OptionView, onClickBtnLeft btnLeft: UIButton) {
        ...
        AlertController.showAlertConfirm(withTitle: "CONFIRM_TITLE".localized, andMessage: "CONFIRM_MESSAGE_SMS_ALERT".localized, alertStyle: .alert, forViewController: self,  lhsButtonTitle: "CANCEL".localized, rhsButtonTitle: "CONFIRM_TITLE_BUTTON_MESSAGE".localized, lhsButtonHandler: nil, rhsButtonHandler: { (action) in
            ...
            Utilities.openSystemApp(type: .message, forController: self, withContent:self.room.phoneContact, completionHander: nil)
        })
    }
    ...
    func optionViewDelegate(view optionView: OptionView, onClickBtnRight btnRight: UIButton) {
        AlertController.showAlertConfirm(withTitle: "CONFIRM_TITLE".localized, andMessage: "CONFIRM_MESSAGE_SMS_ALERT".localized, alertStyle: .alert, forViewController: self, lhsButtonTitle: "CANCEL".localized, rhsButtonTitle: "CONFIRM_TITLE_BUTTON_CALL".localized, lhsButtonHandler: nil, rhsButtonHandler: { (action) in
            ...
            Utilities.openSystemApp(type: .phone, forController: self, withContent: self.room.phoneContact, completionHander: nil)
        })
    }
